...
import { SLIDE_LEFT, SLIDE_RIGHT } from "@/components/FrameMotion";
            ...
            <div className="flex items-center justify-center text-secondary space-x-6 nav-text md:text-[16px] lg:justify-start">
              {planets.map(({ name }, index) => (
                <div
                  ...
                  className={`relative py-2 border-b-2 border-transparent cursor-pointer text-white transition [&:not(.active)]:hover:border-current [&.active]:text-white  ${
                    !index && "active"
                  }`}
                >
                  {name}
